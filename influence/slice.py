...
def colorTif(tifpath,temp):
    ...
    # 找到并读取原始图像
    src_ds = gdal.Open(tifpath,gdalconst.GA_ReadOnly)
    # 转为整型后数据的保存路径
    Int_img_path = g_ifltemp + "RC_org.tif"
    dst_ds = gdal.Translate(Int_img_path, src_ds, outputType=gdalconst.GDT_Int32)
    img = io.imread(Int_img_path)
