...
import time
import evaluateTif as evat
from scipy import stats
import psutil
import pandas as pd
import triggerGeneralize as trig

plt.rcParams['font.sans-serif'] = ['SimHei']


def main(dataFloderPath, ResultfloderPath, TempfloderPath, cell_size):
    index = []
    disatername = dataFloderPath.split('\\')[-1]
    ResultfloderPath = os.path.join(
        ResultfloderPath, str(size))    # --> 初始化文件夹
    TempfloderPath = os.path.join(
        TempfloderPath, str(size))
    ...
    for tif in tiflist:
        ...
        if name.split('_')[-1] == 'str':
            ...
            index.append(os.path.getsize(tif)/1024/1024)
            # 判断数据是连续的还是离散的
            ...
            if condition:
                ...
                core_reclass = func.reclass(
                    core_img, 5, '12')    # 自然断点法
                # core_reclass = func.reclass(core_img, 5)    #等间隔

                mode = stats.mode(
                    core_reclass[core_reclass > 0], keepdims=True)
                func.WriteTifImg(core_reclass_tif_path, proj,
                                 geotrans, core_reclass)
                # 开始综合
                closed_img = func.LayerProcess(
                    core_reclass, outline_img, k_size)
                ...
                # 凸包转shp
                func.TiftoShp(hull_tif_path, 'Value', hull_shp_path)
                show_img = func.LayerProcess2_noremove(
                    core_reclass, outline_img, k_size)
                ...
                eval_list = evat.evaluateLvTif(
                    core_reclass_tif_path, show_tif_path, TempfloderPath)
            else:
                ...
                if len(np.unique(img)) < 10:
                    ...
                    func.WriteTifImg(result_tif_path, proj,
                                     geotrans, img, proj)
                    # 将原图转换成shp,用于语义提取
                    func.TiftoShp(result_tif_path, 'Value', result_shp_path)
                    # 外部轮廓
                    ...
                    # 凸包转tif
                    func.WriteTifImg(hull_tif_path, proj,
                                     geotrans, img_ch, proj)
                    # 凸包转shp
                    func.TiftoShp(hull_tif_path, 'Value', hull_shp_path)
                else:
                ...
                func.delete_shp(result_shp_path)
                eval_list = evat.evaluateLvTif(
                    result_tif_path, result_tif_path, TempfloderPath)
    end = time.time()
    runtime = end-start
    print(runtime)
    index.append(psutil.virtual_memory().percent)
    index.extend(eval_list)
    index.append(runtime)
    index.append(psutil.cpu_percent(interval=runtime))
    index.append(os.path.getsize(result_tif_path)/1024/1024)
    index.append(result_tif_path)
    return index


path = r"F:\Unusual\Group3strenth1207\group3_3857\glass-fire\str"
tempath = r"F:\Unusual\Group3strenth1207\0115_zuhui2\temp"
resultpath = r"F:\Unusual\Group3strenth1207\0115_zuhui2\result"
# print(main(path, resultpath, tempath))

sizeList = [3,5,13,25,63]    #分别表示1：10、1：25、1：50、1：100、1：250
indexl=[]
for size in sizeList:
    indexlist = main(path, resultpath, tempath, size)
...
indexl.to_excel(r'F:\Unusual\Group3strenth1207\0115_zuhui2\index.xlsx')
